...
App.telas['desempenho-cargos'] = function () {
          ...
          <tbody>
            ${linhas.map(l => `
              <tr ${l.exc ? 'class="linha-excecao"' : ''}>
                ...
                <td class="num mono ${(l.exc && l.exc.tipo_calculo !== 'VALOR_FIXO' && l.valorExc === null) ? '' : 'atlas-rep'}"><!-- V965: valor mensal em #189AD3 (aviso "Sem produção LC" fica cinza) -->
                  ${renderValorMensal(l)}
                </td>
              </tr>
            `).join('')}
  ...
  function getStyles() {
    return `
        ...
        .cargos-sb-celwrap:not(:last-of-type) .cargos-sb-cel { border-right: 1px solid #eef3f7; }
        .cargos-sb-cel {
        ...
        .cargos-sb-cel:hover, .cargos-sb-cel.ativo, .cargos-sb-cel.aberta { background: #f4fafd; }
        .cargos-sb-tile {
          width: 28px; height: 28px; border-radius: 8px; flex-shrink: 0;
          display: flex; align-items: center; justify-content: center;
          background: #dbeef8; color: #1c6fa8;
        }
        .cargos-sb-tx { display: flex; flex-direction: column; gap: 1px; flex: 1; min-width: 0; }
        .cargos-sb-rot { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .09em; color: #5b6c7c; white-space: nowrap; }
        .cargos-sb-val { font-size: 13px; font-weight: 700; color: #14384f; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .cargos-sb-chev { color: #7d8fa0; flex-shrink: 0; display: flex; transition: transform 140ms; }
        ...
        .cargos-sb-painel {
          ...
          box-shadow: 0 18px 44px -14px rgba(15,37,68,.42); overflow: hidden;
        }
        .cargos-sb-lista { max-height: 262px; overflow-y: auto; padding: 6px; }
        .cargos-sb-it { display: flex; align-items: center; height: 36px; padding: 0 10px; border-radius: 8px; cursor: pointer; font-size: 13px; color: #14384f; }
        .cargos-sb-it:hover { background: #f2f7fb; }
        .cargos-sb-it.sel { background: #eaf4fb; font-weight: 700; }
