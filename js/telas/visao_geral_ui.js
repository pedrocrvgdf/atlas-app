...
App.telas['dashboard'] = function () {
   ...
   * Agora o rótulo é um <span> próprio, sempre em #6b7075 (--ink-faint, o mesmo
          ...
          <table class="vg-esp-tab" id="vg-esp-tab">
            ...
            <tfoot><tr>
              ...
              ${modoPerda
                ...
                : `<td class="num vg-esp-rep">${moeda(T.repasse)}<span class="vg-esp-pct">${pct(T.repasse, T.recebido)}</span></td>`}<!-- V963: total do repasse em #46688c -->
            </tr></tfoot>
          </table>
        </div>
...
function injetarEstilosVG() {
  ...
  s.textContent = `
    ...
    .vg-cmp-chip:hover { border-color: #3f6489; color: #46688c; }
    .vg-cmp-chip.on { background: #46688c; border-color: #46688c; color: #fff; }
    ...
    .vg-glosa-pct { font-size: 11px; font-weight: 800; color: #a15646; vertical-align: middle; margin-left: 8px; }
    ...
    .vg-glosa-rot { font-size: 10px; font-weight: 800; letter-spacing: .05em; text-transform: uppercase; color: #a15646; line-height: 1.3; }
    /* V692: 14px a pedido · V922: valor e % SEMPRE na mesma linha (sem quebrar) */
    .vg-glosa-vlr { font-size: 14px; font-weight: 800; color: #a15646; line-height: 1.25; white-space: nowrap; }
    ...
    .vg-glosa-sw:hover { color: #a15646; border-color: #E4C4C0; }
    .vg-glosa-sw-on { background: #a15646; color: #FFF; border-color: #a15646; }
    .vg-glosa-sw-on:hover { color: #FFF; }
    .vg-glosa-sw:focus-visible { outline: 2px solid #a15646; outline-offset: 2px; }
    /* V531: botão slim de export do relatório de glosa (canto inferior direito) */
    .vg-glosa-export { flex: none; width: 26px; height: 26px;
      border-radius: 7px; border: 1px solid #E4C4C0; background: #fff; color: #a15646;
      display: inline-flex; align-items: center; justify-content: center; cursor: pointer;
      font-size: 14px; line-height: 1; padding: 0;
      transition: background-color 130ms, color 130ms, border-color 130ms, transform 130ms; }
    .vg-glosa-export:hover { background: #a15646; color: #fff; border-color: #a15646; transform: translateY(-1px); }
    ...
    .vg-cmp-bloco-rep { background: #f7f8fa; }
    ...
    .vg-aic .aic-lbl { color: #1d1f20; }   /* V849: título dos cards totalizadores */   /* rótulo dos cards (PRODUÇÃO TOTAL, REPASSE TOTAL…) */
    ...
    #vg-filtros.vg-preso { background: linear-gradient(to bottom, var(--bg, #fafbfc) 0%, var(--bg, #fafbfc) 78%, rgba(245,250,253,0) 100%); padding-bottom: 10px; }
    #vg-filtros.vg-preso .vg-sb { margin-bottom: 0; box-shadow: 0 2px 4px rgba(89, 128, 166,.06), 0 18px 34px -16px rgba(89, 128, 166,.35); border-color: #d6e2eb; }
    .vg-sb { display: flex; align-items: stretch; background: #fff; border: 1px solid #eef2f6; border-radius: 14px;
      padding: 6px; margin: 4px 0 20px; position: relative; z-index: 30;
      box-shadow: 0 1px 2px rgba(89, 128, 166,.04), 0 10px 28px -20px rgba(89, 128, 166,.18); }
    .vg-sb-celwrap { position: relative; display: flex; min-width: 0; }
    .vg-sb-div { width: 1px; background: #f7f8fa; margin: 6px 0; flex: none; }
    ...
    .vg-sb-cel:hover, .vg-sb-cel.ativo, .vg-sb-cel.aberto { background: #fafbfc; }
    .vg-sb-cel:focus-visible { outline: 2px solid #3f6489; outline-offset: 2px; }
    .vg-sb-tile { width: 30px; height: 30px; flex: none; border-radius: 8px; background: #f0f5f9; color: #585d62;
      display: flex; align-items: center; justify-content: center; }
    .vg-sb-cel.aberto .vg-sb-tile, .vg-sb-cel.ativo .vg-sb-tile { background: #eef2f6; color: #46688c; }
    .vg-sb-txt { display: flex; flex-direction: column; gap: 1px; min-width: 0; flex: 1; }
    .vg-sb-rot { font-size: 9.5px; font-weight: 700; letter-spacing: .11em; text-transform: uppercase; color: #8a9096; }
    .vg-sb-cel.aberto .vg-sb-rot { color: #46688c; }
    .vg-sb-val { font-size: 13.5px; font-weight: 700; color: #3a5877; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .vg-sb-chev { flex: none; color: #8a9096; display: flex; transition: transform .15s; }
    .vg-sb-cel.aberto .vg-sb-chev { color: #46688c; transform: rotate(180deg); }
    .vg-sb-acao { flex: none; display: flex; align-items: center; padding: 0 6px 0 14px; }
    .vg-sb-btn { height: 50px; padding: 0 18px; border: none; border-radius: 10px; background: #46688c; color: #fff;
    ...
    .vg-sb-btn:hover { background: #5980a6; }
    .vg-sb-conta-slot { height: 50px; display: flex; align-items: center; padding: 0 14px; font-size: 13px;
      font-weight: 700; color: #585d62; white-space: nowrap; font-variant-numeric: tabular-nums; }
    ...
    .vg-sb-busca { flex: 1; min-width: 0; border: none; background: none; font: inherit; font-size: 13.5px; color: #3a5877; }
    .vg-sb-busca:focus { outline: none; }
    .vg-sb-busca::placeholder { color: #8a9096; }
    .vg-sb-busca-x { width: 20px; height: 20px; flex: none; border: none; border-radius: 999px; background: #eef0f2;
      color: #585d62; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0; }
    .vg-sb-lista { max-height: 262px; overflow-y: auto; padding: 6px; border-top: 1px solid #f7f8fa; }
    .vg-sb-painel > .vg-sb-lista:first-child { border-top: none; }
    .vg-sb-lista::-webkit-scrollbar { width: 8px; }
    .vg-sb-lista::-webkit-scrollbar-track { background: #f7f8fa; }
    .vg-sb-lista::-webkit-scrollbar-thumb { background: #e4eaf1; border-radius: 4px; }
    .vg-sb-it { height: 38px; display: flex; align-items: center; gap: 10px; padding: 0 8px; border-radius: 8px;
      cursor: pointer; font-size: 13.5px; font-weight: 500; color: #3a5877; }
    .vg-sb-it:hover, .vg-sb-it.hl { background: #f7f8fa; }
    .vg-sb-it.sel { background: #f7f8fa; }
    ...
    .vg-sb-chip { width: 24px; height: 24px; flex: none; border-radius: 999px; background: #f7f8fa; color: #46688c;
      font-size: 9.5px; font-weight: 700; display: flex; align-items: center; justify-content: center; letter-spacing: .02em; }
    .vg-sb-ck { flex: none; color: #46688c; display: flex; }
    .vg-sb-vazio { font-size: 13px; color: #8a9096; padding: 18px 10px; }
    .vg-sb-rodape { display: flex; align-items: center; justify-content: space-between; gap: 10px;
      padding: 9px 13px; border-top: 1px solid #f7f8fa; background: #fbfdfe; font-size: 11.5px; color: #585d62; }
    .vg-sb-dica { font-size: 11px; color: #8a9096; }
    ...
    .vg-aic { position: relative !important; height: auto !important; display: flex;
      ...
      box-shadow: -4px -3px 10px rgba(89, 128, 166,.30), 0 8px 18px rgba(29, 31, 32,.13); }
    ...
    .vg-aic { background:
        ...
        linear-gradient(90deg, #5980a6 0%, #46688c 34%, #3f6489 68%, #d8e4ef 100%) border-box;
    ...
    .vg-aic-glosa { box-shadow: -4px -3px 0 0 #c0563f, -4px -3px 10px rgba(192,86,63,.28), 0 8px 18px rgba(29, 31, 32,.13); }
    ...
    /* V715: o !important é NECESSÁRIO — o style.css tem uma regra global
       ".main [class*=comp-up] { color:#4f8a5b !important }" (V181) que vencia
       estas cores e mantinha o verde antigo na VG. */
    .vg-var.comp-up { color: #15a34a !important; }   /* V711 (era #4f8a5b) */
    .vg-var.comp-down { color: #a15646 !important; }
    ...
    /* V863: o rótulo NÃO acompanha a cor do indicador — fica sempre no cinza
       #6b7075, como nos demais cards. Precisa da mesma força das regras acima
    ...
    .vg-var.vg-var-lm .vg-var-lbl { color: #6b7075 !important; font-weight: 600; }
    ...
    .vg-evolucao-bloco { background: #fff; border: 1px solid #eceff2; border-radius: 16px;
      ...
      box-shadow: 0 1px 2px rgba(29, 31, 32,.04), 0 12px 32px -18px rgba(29, 31, 32,.16); }
    ...
    .vg-bloco { background: #fff; border: 1px solid var(--border); border-radius: 14px; padding: 18px 20px; margin-bottom: 20px; box-shadow: 0 6px 16px rgba(29, 31, 32,.05); }
    ...
    .vg-evo-ano:focus { outline: none; border-color: #5980a6; box-shadow: 0 0 0 3px rgba(89, 128, 166,.14); }
    .vg-bar { fill: #1d1f20; transition: fill .15s; }
    .vg-bar-g:hover .vg-bar { fill: #5980a6; }
    .vg-bar-atual { fill: #5980a6 !important; }
    ...
    .vg-esp-export { border: 1px solid #eef0f2; background: #fff; border-radius: 9px; padding: 7px 12px; font-size: 12px; font-weight: 700; color: #5980a6; cursor: pointer; }
    .vg-esp-export:hover { background: #f7f8fa; border-color: #3f6489; }
    ...
    .vg-esp-tab th { background: #46688c; color: #fff; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .3px; padding: 9px 12px; text-align: left; white-space: nowrap; }
    ...
    .vg-esp-tab td { padding: 8px 12px; border-bottom: 1px solid #EDF2F0; color: #3a5877; vertical-align: top; }
    .vg-esp-tab tbody tr:hover td { background: #fafbfc; }
    .vg-esp-nome { font-weight: 700; color: #1d1f20; }
    .vg-esp-adm { display: block; font-size: 10.5px; font-weight: 500; color: #585d62; font-family: var(--font-body, inherit); }
    .vg-esp-pct { display: block; font-size: 10.5px; color: #585d62; font-family: var(--font-body, inherit); }
    .vg-esp-glosa { color: #a15646; font-weight: 400; }   /* V956: sem negrito na Glosa */
    ...
    .vg-esp-perdido, .vg-esp-sub td.num.vg-esp-perdido { color: #c07a66 !important; font-weight: 700; }
    .main .vg-esp-tab tfoot td.vg-esp-perdido, .main .vg-esp-tab tfoot td.vg-esp-perdido * { color: #c07a66 !important; }
    /* V934: sem a tarja preta global do style.css (.main table tfoot td) — rodapé claro, só em negrito */
    .main .vg-esp-tab tfoot td, .main .vg-esp-tab tfoot tr:hover td { font-weight: 800; background: #F6F9FB !important; color: #1d1f20 !important; border-top: 2px solid #eef0f2 !important; border-bottom: none !important; border-color: #eef0f2 !important; }
    .main .vg-esp-tab tfoot td * { color: #585d62 !important; }
    /* V934: drilldown Categoria › Subcategoria */
    .vg-esp-cat-dd { cursor: pointer; }
    .vg-esp-seta { display: inline-block; width: 14px; color: #46688c; font-size: 11px; }
    .vg-esp-aberta td { background: #fafbfc; }
    ...
    .vg-esp-sub td.num.vg-esp-glosa { color: #a15646; }
    ...
    .vg-il-mk-glo { background: #e0483f; } .vg-il-mk-rep { background: #5980a6; }   /* V547: ciano */
    ...
    /* V953: a numeração do Top 10 ganhou o MESMO efeito da Performance médica
       (círculo claro #eef2f5, número #585d62) — antes era marinho com branco */
    .vg-il-pos-rank, .vg-il-pos-neutra { background: #eef2f5; color: #585d62; }
    ...
    .vg-il-seg-glo { background: #e0483f; } .vg-il-seg-rep { background: #5980a6; }   /* V547: ciano */
    ...
    .vg-dr-card { background: #fff; border: 1px solid #e4eaef; border-radius: 8px;
      ...
      box-shadow: 0 1px 2px rgba(29, 31, 32,.05), 0 12px 28px -20px rgba(29, 31, 32,.28); }
    .vg-dr-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; margin-bottom: 10px; }
    .vg-dr-titulo { font-size: 16px; font-weight: 700; color: #3a5877; letter-spacing: -.005em; }
    .vg-dr-sub { font-size: 12px; color: #8a9096; margin-top: 3px; }
    .vg-dr-leg { display: flex; gap: 20px; align-items: center; flex-wrap: wrap;
      font-size: 12px; font-weight: 600; color: #3a5877;
    ...
    .vg-dr-sw { width: 11px; height: 11px; border-radius: 2px; flex: none; display: inline-block;
      box-shadow: 0 1px 2px rgba(29, 31, 32,.25); }
    .vg-dr-sw-des { background: linear-gradient(180deg, #d8e4ef, #1aa9c9); }
    ...
    .vg-dr-ylbl { position: absolute; right: 10px; font-size: 10.5px; font-weight: 600; color: #8a9096;
    ...
    .vg-dr-val-des { color: #3a5877; }
    .vg-dr-val-rep { color: #5980a6; }
    ...
    .vg-dr-bar-des { background: linear-gradient(180deg, #d8e4ef 0%, #29c0e0 55%, #1aa9c9 100%);
    ...
    .vg-dr-pill-zero { background: #8a9096; }
    ...
    .vg-gc-legenda { display: flex; gap: 16px; align-items: center; font-size: 11.5px; color: #585d62; flex-wrap: wrap; margin-bottom: 14px; }
    ...
    .vg-gc-mes { width: 88px; flex-shrink: 0; text-align: center; font-size: 13px; font-weight: 600; color: #585d62; }
    ...
    .vg-rank-pos { width: 26px; height: 26px; flex: 0 0 26px; border-radius: 50%; background: #1d1f20; color: #fff; font-size: 12px; font-weight: 800; display: flex; align-items: center; justify-content: center; }
    ...
    .vg-rank-val { font-size: 13px; font-weight: 800; color: #5980a6; white-space: nowrap; }
    .vg-val-glosa { color: #A35A2A; }
    .vg-rank-barra { height: 6px; background: #EDF1EF; border-radius: 4px; margin: 4px 0 2px; overflow: hidden; }
    .vg-rank-fill { height: 100%; background: linear-gradient(90deg, #5980a6, #5980a6); border-radius: 4px; }
