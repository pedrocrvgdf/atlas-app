...
(function () {
  ...
  const CATS = {
    regra:     { nome: 'Mudança de regra',   cor: '#1c6fa8', tint: '#eaf4fb', texto: '#14547f' },
  ...
  const SINAL_COR = { alta: '#1d8f5f', baixa: '#c0563f', neutro: '#5b6c7c' };
  ...
  function injetarEstilos() {
    ...
    st.textContent = `
      ...
      .tl-faixa { background: linear-gradient(90deg, #16456b 0%, #1d6d92 34%, #2a9fc4 68%, #4fd3ec 100%);
        border-radius: 14px; box-shadow: 0 1px 2px rgba(20,50,80,.05), 0 14px 32px -22px rgba(16,42,66,.5);
      ...
      .tl-novo { flex-shrink: 0; font: 700 12.5px/1 'Inter Tight', -apple-system, sans-serif;
        color: #16456b; background: #fff; border: none; border-radius: 9px; padding: 9px 14px;
        cursor: pointer; white-space: nowrap; box-shadow: 0 2px 6px rgba(10,40,66,.2);
      ...
      .tl-marco.tl-marco-alta { box-shadow: 0 0 0 3px rgba(184,150,90,.45), 0 2px 8px rgba(10,40,66,.3); }
      ...
      .tl-lupa.on { background: #fff; color: #16456b; box-shadow: 0 2px 6px rgba(10,40,66,.25); }
      .tl-lupa-pop { position: fixed; z-index: 100060; width: 190px; background: #fff;
        border: 1px solid #dfe8f0; border-radius: 10px; overflow: hidden;
        box-shadow: 0 14px 36px -10px rgba(15,37,68,.4); }
      .tl-lupa-tit { font-size: 10px; font-weight: 700; letter-spacing: .09em; text-transform: uppercase;
        color: #90a1ae; padding: 9px 12px 7px; border-bottom: 1px solid #edf2f6; }
      ...
      .tl-lupa-busca input { flex: 1; min-width: 0; border: none; background: none; font: 500 12.5px/1 inherit;
        font-family: inherit; color: #14384f; }
      .tl-lupa-busca input:focus { outline: none; }
      .tl-lupa-busca input::placeholder { color: #9aabb8; }
      .tl-lupa-busca button { width: 18px; height: 18px; flex: none; border: none; border-radius: 999px;
        background: #dfe8f0; color: #5b6c7c; font-size: 10px; line-height: 1; cursor: pointer; padding: 0; }
      ...
      .tl-lupa-it { width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 8px;
        border: none; background: none; font: 600 12.5px/1 inherit; font-family: inherit; color: #14384f;
        padding: 8px 9px; border-radius: 7px; cursor: pointer; text-align: left; }
      .tl-lupa-it:hover { background: #f2f7fb; }
      .tl-lupa-it.sel { background: #eaf4fb; font-weight: 700; }
      .tl-lupa-qt { font-size: 10.5px; font-weight: 700; color: #1c6fa8; background: #eaf4fb;
      ...
      .tl-scroller { flex: 1; min-width: 0; overflow-x: auto; overflow-y: hidden;
        scrollbar-width: thin; scrollbar-color: #c3d5e2 transparent; }
      ...
      .tl-scroller::-webkit-scrollbar-thumb { background: #c3d5e2; border-radius: 4px; }
      ...
      .tl-marco { width: 24px; height: 24px; padding: 0; cursor: pointer;
        ...
        transform: rotate(-45deg); box-shadow: 0 3px 7px rgba(10,40,66,.3);
        position: relative; z-index: 2; transition: transform .12s ease; }
      .tl-marco:hover { transform: rotate(-45deg) scale(1.16); }
      .tl-marco:focus-visible { outline: 2px solid #2f8fc4; outline-offset: 2px; }
      ...
      .tl-balao { position: fixed; transform: translateX(-50%);
        ...
        box-shadow: 0 6px 18px rgba(10,40,66,.32); pointer-events: none; text-align: left; }
      ...
      .tl-balao-tit { font: 700 12px/1.3 'Inter Tight', -apple-system, sans-serif;
        color: #14384f; text-wrap: pretty; display: -webkit-box; -webkit-line-clamp: 2;
        -webkit-box-orient: vertical; overflow: hidden; }

      /* ── modal (9D) ─────────────────────────────────────────────────── */
      .tl-ov { position: fixed; inset: 0; z-index: 100000; background: rgba(15,37,68,.5);
      ...
      .tl-modal-head { padding: 20px 24px 16px; border-bottom: 1px solid #edf2f6;
      ...
      .tl-modal-data { font-size: 12.5px; font-weight: 600; color: #5b6c7c; }
      .tl-x { width: 30px; height: 30px; border-radius: 8px; border: none; background: #f2f6f9;
        color: #5b6c7c; font-size: 17px; line-height: 1; cursor: pointer; padding: 0;
        transition: background-color .12s ease, color .12s ease; }
      .tl-x:hover { background: #e5edf3; color: #14384f; }
      .tl-lapis { display: inline-flex; align-items: center; justify-content: center;
        width: 30px; height: 30px; border: none; border-radius: 8px;
        background: #f2f6f9; color: #5b6c7c; cursor: pointer;
        transition: color .12s ease, background-color .12s ease; }
      .tl-lapis:hover { color: #1c6fa8; background: #eaf4fb; }
      .tl-ver-titulo { font: 700 20px/1.25 'Inter Tight', -apple-system, sans-serif;
        letter-spacing: -.01em; color: #14384f; text-wrap: pretty; }
      ...
      .tl-cartao { background: #f7fafc; border: 1px solid #e8eff5; border-radius: 10px;
        padding: 11px 13px; display: flex; flex-direction: column; gap: 2px; min-width: 0; }
      .tl-cartao-v { font: 700 14.5px/1.3 'Inter Tight', -apple-system, sans-serif; color: #14384f;
      ...
      .tl-lbl { font: 700 10px/1 'Inter Tight', -apple-system, sans-serif; text-transform: uppercase;
        letter-spacing: .1em; color: #90a1ae; }
      .tl-ver-desc, .tl-in { font: inherit; font-size: 13.5px; line-height: 1.55; color: #1b2a3a;
        background: #f7fafc; border: 1px solid #dfe8f0; border-radius: 10px; padding: 10px 13px;
        width: 100%; box-sizing: border-box; }
      .tl-ver-desc { resize: vertical; }
      textarea.tl-in { resize: vertical; }
      .tl-ver-desc:focus, .tl-in:focus { outline: none; border-color: #2f8fc4;
      ...
      .tl-ico-op { width: 34px; height: 34px; display: inline-flex; align-items: center;
        justify-content: center; border: 1px solid #dfe8f0; border-radius: 9px;
        background: #f7fafc; color: #5b6c7c; cursor: pointer; padding: 0;
        transition: border-color .12s ease, background-color .12s ease, color .12s ease; }
      .tl-ico-op:hover { border-color: #2f8fc4; color: #1c6fa8; }
      .tl-ico-op.on { border-color: #1c6fa8; background: #eaf4fb; color: #1c6fa8;
        box-shadow: 0 0 0 1px #1c6fa8 inset; }
      ...
      .tl-docs-n { font-size: 11.5px; font-weight: 600; color: #5b6c7c; }
      .tl-docs-lista { display: flex; flex-direction: column; gap: 7px; }
      .tl-doc { display: flex; align-items: center; gap: 11px; background: #f7fafc;
      ...
      .tl-doc-nome { flex: 1; min-width: 0; text-align: left; font-size: 13px; font-weight: 600;
        color: #14384f; background: none; border: none; padding: 0; cursor: pointer;
        overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .tl-doc-nome:hover { color: #1c6fa8; text-decoration: underline; }
      .tl-doc-tam { font-size: 11.5px; color: #5b6c7c; white-space: nowrap; }
      ...
      .tl-doc-vazio { font-size: 12.5px; color: #90a1ae; background: #f7fafc;
        border: 1px dashed #dfe8f0; border-radius: 10px; padding: 14px 13px; }
      .tl-anexar { align-self: flex-start; display: inline-flex; align-items: center; gap: 8px;
        font: 700 12.5px/1 'Inter Tight', -apple-system, sans-serif; color: #1c6fa8;
        background: #eaf4fb; border: 1px solid #cbe3f2; border-radius: 10px; padding: 10px 14px;
        cursor: pointer; transition: background-color .12s ease; }
      .tl-anexar:hover { background: #d8ebf7; }
      .tl-modal-foot { padding: 14px 24px; border-top: 1px solid #edf2f6;
        display: flex; align-items: center; gap: 9px; }
      .tl-foot-info { margin-left: auto; font-size: 11.5px; color: #90a1ae; }
      .tl-salvar { font: 700 12.5px/1 'Inter Tight', -apple-system, sans-serif; color: #fff;
        background: #1c6fa8; border: none; border-radius: 9px; padding: 11px 18px; cursor: pointer;
        transition: background-color .12s ease; }
      .tl-salvar:hover { background: #14547f; }
      .tl-cancelar { font: 700 12.5px/1 'Inter Tight', -apple-system, sans-serif; color: #5b6c7c;
        background: transparent; border: 1px solid #dfe8f0; border-radius: 9px; padding: 10px 16px;
        cursor: pointer; transition: border-color .12s ease, color .12s ease; }
      .tl-cancelar:hover { border-color: #c3d5e2; color: #14384f; }
