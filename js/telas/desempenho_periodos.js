...
App.telas['desempenho-periodos'] = function () {
  ...
  function renderTabela(linhas, mes_ref, cfgCols) {
    ...
    function renderTr(l) {
      ...
      const tds = cols.map(c => {
        ...
        if (c.id === 'total_valor')    return `<td class="num mono atlas-rep" ${attr}>R$ ${fmt(l.total_valor, 2)}</td>`;   /* V962: valor de repasse em #107DAC */
        return '<td></td>';
      }).join('');
    ...
    function renderGrupo(unidade, linhasUnidade) {
       ...
       * Antes era dourado #B8965A sobre #06283A.
       */
   ...
   * ESTILIZADA via ExcelJS (cabeçalho #107DAC, subtotais em negrito, moeda).
  ...
  function getStyles() {
    return `
        ...
        .btn-compact-primary:hover { background: #003A54; border-color: #003A54; color: #E8F1F7; }
        ...
        .per-foco-banner strong { color: #003A54; }
        ...
        .per-sb-celwrap:not(:last-child) .per-sb-cel { border-right: 1px solid #eef3f7; }
        .per-sb-cel {
          flex: 1; min-width: 0;
        ...
        .per-sb-cel:hover, .per-sb-cel.ativo, .per-sb-cel.aberta { background: #f4fafd; }
        .per-sb-cel:focus-visible { outline: 2px solid #2f8fc4; outline-offset: 2px; }
        .per-sb-tile {
          width: 28px; height: 28px; border-radius: 8px; flex-shrink: 0;
          display: flex; align-items: center; justify-content: center;
          background: #f0f5f9; color: #5b6c7c;
        }
        .per-sb-cel.ativo .per-sb-tile, .per-sb-cel.aberta .per-sb-tile { background: #dbeef8; color: #1c6fa8; }
        ...
        .per-sb-cel.ativo .per-sb-val { font-weight: 700; color: #14384f; }
        .per-sb-chev { color: #7d8fa0; flex-shrink: 0; display: flex; transition: transform 140ms; }
        ...
        .per-sb-busca::placeholder { color: #9aabb8; }
        .per-sb-busca:focus { border-color: #2f8fc4; }
        .per-sb-lista { max-height: 262px; overflow-y: auto; padding: 6px; }
        .per-sb-lista::-webkit-scrollbar { width: 8px; }
        .per-sb-lista::-webkit-scrollbar-track { background: #f2f6f9; }
        .per-sb-lista::-webkit-scrollbar-thumb { background: #c3d5e2; border-radius: 4px; }
        .per-sb-it {
          display: flex; align-items: center; gap: 10px;
          height: 38px; padding: 0 8px; border-radius: 8px; cursor: pointer;
          font-size: 13px; color: #14384f;
        }
        .per-sb-it:hover, .per-sb-it.foco { background: #f2f7fb; }
        .per-sb-it.sel { background: #eaf4fb; font-weight: 700; }
        ...
        .per-sb-ck { color: #1c6fa8; display: flex; }
        .per-sb-rodape {
        ...
        .per-card-verde   { background: linear-gradient(180deg, #DBF0F9 0%, #EDF5F0 100%); }
        .per-card-verde .per-card-faixa { background: #003A54; }
        ...
        .per-card-bege .per-card-faixa { background: #005073; }
        .per-card-destaque { background: #003A54; color: #56645E; }
        .per-card-destaque .per-card-titulo { color: #56645E; }
        .per-card-destaque .per-card-valor { color: #005073; }
        .per-card-destaque .per-card-sub { color: #56645E; }
        ...
        .per-grupo-header td {
          background: #EAF4FB;   /* V863 (era #8FABC4) */
          font-weight: 700;
          color: #06283A;
          font-size: 11px;
        ...
        /* V861/V863: glifo #B8965A sobre pastilha #06283A. O dourado direto na
        ...
        .per-grupo-header:hover td { background: #EAF4FB; }
        .per-grupo-icone {
          margin-right: 6px;
        ...
        .per-ext-op:has(input:checked) { background: #E4EEF4; border-color: #107DAC; }
        .per-ext-op input { margin-top: 2px; accent-color: #107DAC; }
