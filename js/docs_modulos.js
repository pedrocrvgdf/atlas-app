...
(function () {
  ...
  window.AtlasDocs['externos'] = {
    titulo: 'Matriz Externa',
    secoes: [
      { titulo: 'O que este módulo faz', tipo: 'paragrafo', conteudo: [
        'Calcula o **repasse de médicos externos e híbridos** — os que encaminham pacientes de fora para a casa. A lógica é diferente da do repasse interno: aqui não se paga por procedimento de tabela, e sim um **percentual sobre o líquido da conta**, que é o faturamento da admissão menos o que não pertence ao médico (material, lente, imposto).',
        'O ponto central do desenho é a **independência**: a lista de admissões chega de fora, mas **os valores não**. Todo número é recalculado cruzando a admissão com a **PRODUÇÃO** da própria casa. Isso tira da conta qualquer valor digitado por terceiro.',
        'Duas decisões mudam bastante o resultado e por isso ficam explícitas: se a LIO é **ATLAS ou PARCERIA**, e se o médico é **EXTERNO ou HÍBRIDO**.'
      ] },
      { titulo: 'Como operar (passo a passo)', tipo: 'passos', conteudo: [
        'Importe primeiro a **PRODUÇÃO** do mês (modo normal → Importar → Produção). Sem ela, nenhuma admissão externa tem com o que ser cruzada.',
        'Em **Base de Cálculo**, confira MAT/MED e HM por procedimento, o custo das LIOs, o custo dos OPMEs e as clínicas. A regra do processo é **revisar a cada 6 meses**.',
        'Em **Médicos & Clínicas**, confira o **vínculo** de cada um: EXTERNO ou HÍBRIDO. É o que decide o destino do HM.',
        'Em **Importar**, traga a planilha de admissões do espaço verde e confirme a prévia.',
        'Na **Matriz Externa**, resolva as pendências da coluna MÉDICO: nome de clínica com vários médicos pede **Escolher médico**; nome desconhecido pede **＋ Cadastrar**.',
        'Marque, em cada admissão com LIO, se é **ATLAS** ou **PARCERIA**. A ferramenta sugere, mas a decisão é sua e fica gravada.',
        'Confira as deduções linha a linha, principalmente OPME e LIO.',
        'Vá para **Relatórios Externos** e gere a extração por médico.'
      ] },
      { titulo: 'De onde vêm os dados', tipo: 'lista', conteudo: [
        'A **planilha de admissões** enviada pelo espaço verde — dela só se aproveita **a lista de admissões**, não os valores.',
        '**linhas_producao** (PRODUÇÃO) — a fonte real de todos os números: produzido, produtos, cirurgião.',
        '**Base de Cálculo dos Externos** — MAT/MED e HM por procedimento, custo das LIOs, custo dos OPMEs e as clínicas.',
        '**Cadastro de Médicos + De-Para** — para resolver o nome que veio da produção.',
        '**Ajustes** (no cabeçalho) — os percentuais de imposto, de indicação e de indicação com LIO ATLAS.'
      ] },
      { titulo: 'Como é calculado', tipo: 'lista-ordenada', conteudo: [
        '**FAT TOTAL** = soma do PRODUZIDO das linhas da admissão na PRODUÇÃO.',
        'Deduções: **OPME** (linhas classificadas como OPME ou com valor batendo na tabela de OPMEs), **LIO** (linha cujo valor bate com o custo de alguma lente da Base de Cálculo; sem valor batendo, cruza pelo **nome mais próximo**), **imposto** (10% do FAT) e **MAT/MED** do procedimento-base.',
        '**Líquido** = FAT − deduções → **indicação** = líquido × 20% (18% quando a LIO é valor ATLAS).',
        '**LIO — marcação ATLAS × PARCERIA (V840)**: toda admissão com LIO pede a marcação nas contas (a ferramenta **sugere**: bateu pelo valor da base = PARCERIA). **PARCERIA** deduz o custo da lente e paga 20%; **ATLAS** não deduz a lente e paga 18%. A escolha fica gravada na admissão.',
        '**HM**: médico **EXTERNO** recebe o HM (soma no repasse final); médico **HÍBRIDO** tem o HM nas **deduções** — ele o recebe pela folha interna, então o valor sai do líquido antes do percentual (V828). **V840**: o HM segue o **cirurgião da produção** — só é pago quando o próprio médico operou (cirurgião interno/outro → só a indicação; produção sem cirurgião mantém o HM).',
        '**REPASSE** = indicação + HM (o HM só na ponta do EXTERNO).'
      ] },
      { titulo: 'Memória de cálculo', tipo: 'formula', conteudo: [
        ['1. Faturamento da admissão', 'FAT TOTAL = soma do PRODUZIDO de todas as linhas daquela\n            admissão na PRODUÇÃO'],
        ['2. Deduções', 'OPME     = linhas classificadas como OPME, ou cujo valor\n           bate com a tabela de OPMEs\nLIO      = custo da lente (só quando PARCERIA)\nimposto  = 10% do FAT TOTAL\nMAT/MED  = valor do procedimento-base na Base de Cálculo\nHM       = entra aqui SÓ quando o médico é HÍBRIDO\n\ndeduções = OPME + LIO + imposto + MAT/MED (+ HM se híbrido)'],
        ['3. Líquido', 'líquido = FAT TOTAL − deduções'],
        ['4. Indicação', 'indicação = líquido × 20%\n\nExceção: quando a LIO é ATLAS → 18%\n(não se deduz a lente, então o percentual é menor)'],
        ['5. Repasse final', 'repasse = indicação + HM\n\nO HM só entra aqui quando o médico é EXTERNO.\nNo HÍBRIDO ele já foi deduzido no passo 2 — porque ele\nrecebe o HM pela folha interna, e pagar de novo seria\nduplicar.'],
        ['Quando o HM é pago', 'O HM segue o CIRURGIÃO da produção: só é pago quando o\npróprio médico operou.\n\ncirurgião = outro médico  → paga só a indicação\nprodução sem cirurgião    → mantém o HM'],
        ['Procedimento AO (ambos os olhos)', 'MAT/MED e HM dobram — ou usam a coluna AO da Base de\nCálculo, quando ela estiver preenchida.']
      ] },
      { titulo: 'Exemplo numérico', tipo: 'exemplo', conteudo: [
        '**Admissão com FAT TOTAL de R$ 10.000,00**, médico **EXTERNO**, que operou, com LIO marcada como **PARCERIA**:',
        'Deduções: OPME R$ 0,00 + LIO R$ 2.000,00 + imposto R$ 1.000,00 (10%) + MAT/MED R$ 500,00 = **R$ 3.500,00**',
        'Líquido = 10.000 − 3.500 = **R$ 6.500,00** → indicação = 6.500 × 20% = **R$ 1.300,00**',
        'HM do procedimento = R$ 800,00, e ele é EXTERNO e operou → **repasse = 1.300 + 800 = R$ 2.100,00**',
        '— — —',
        '**A mesma admissão, mas com a LIO marcada como ATLAS:** a lente **não é deduzida** e o percentual cai para 18%.',
        'Líquido = 10.000 − 1.500 = R$ 8.500,00 → indicação = 8.500 × 18% = **R$ 1.530,00** → repasse = 1.530 + 800 = **R$ 2.330,00**.',
        'É por isso que a marcação ATLAS × PARCERIA precisa ser consciente: ela muda o resultado.'
      ] },
      { titulo: 'Armadilhas e cuidados', tipo: 'lista', conteudo: [
        'Os **valores da planilha recebida não são usados** — só a lista de admissões. Divergência com o que o parceiro enviou é esperada, e a fonte da verdade é a PRODUÇÃO.',
        '**EXTERNO × HÍBRIDO** muda o destino do HM: no externo ele soma no fim; no híbrido ele é deduzido antes do percentual. Vínculo errado distorce o repasse nos dois sentidos.',
        '**ATLAS × PARCERIA** muda dedução e percentual ao mesmo tempo. A sugestão da ferramenta é um palpite pelo valor; confirme.',
        'Sem a **PRODUÇÃO do mês importada**, a matriz não tem como calcular nada.',
        'A **Base de Cálculo desatualizada** silenciosamente erra todas as contas: é ela que fornece MAT/MED, HM e os custos.',
        'Nome de **clínica** com vários médicos vinculados exige escolha manual — e a escolha vale **só para aquela admissão**.'
      ] },
      { titulo: 'Coluna MÉDICO', tipo: 'lista', conteudo: [
        'O nome vem da PRODUÇÃO e é resolvido pelo **cadastro + de-para** (módulo MÉDICOS e sinônimos) — sem depender de conciliação manual.',
        'Nome de **CLÍNICA** (empresa/CNPJ do médico): com um médico vinculado, resolve sozinho; com vários, o botão **Escolher médico** abre o seletor — a escolha vale **só para aquela admissão**.',
        'Médico **desconhecido**: o botão **＋ Cadastrar** cria o registro e abre o mesmo formulário de edição do módulo MÉDICOS, ali mesmo.',
        'As colunas **Proc. (planilha)** e **Produto (produção)** mostram, lado a lado, a nomenclatura enviada na planilha importada e o produto lançado na PRODUÇÃO.'
      ] },
      { titulo: 'Glossário', tipo: 'glossario', conteudo: [
        ['EXTERNO', 'Médico de clínica parceira — recebe 20% + HM quando opera.'],
        ['HÍBRIDO', 'Médico com vínculo interno — recebe os 20% com o HM deduzido do líquido (o HM vem na folha).'],
        ['AO', 'Procedimento nos dois olhos — MAT/MED e HM dobram (ou usam a coluna AO da base).'],
        ['Sem HM', 'Marcação de Ajustes p/ externo que só encaminhou (não operou).']
      ] }
    ]
  };
  window.AtlasDocs['externos-relatorios'] = {
    titulo: 'Relatórios Externos',
    secoes: [
      { titulo: 'Objetivo', tipo: 'paragrafo', conteudo: [
        'O **fim do processo** do modo Externo — como o RELATÓRIOS do ATLAS: o relatório recebido (admissão + nome do paciente) é lido, cada admissão é **cruzada com a PRODUÇÃO** pelo mesmo motor da Matriz, e a extração final sai **por médico**, pronta para envio.'
      ] },
      { titulo: 'Como funciona', tipo: 'lista-ordenada', conteudo: [
        'Importe o relatório (.xlsx/.xls/.csv, estrutura livre) — os números de admissão são achados e validados contra a PRODUÇÃO.',
        'A tela agrupa por **médico** (mesma resolução da Matriz: cadastro, de-para, clínica, escolha por admissão) e mostra a **DESCRIÇÃO (padrão do procedimento)** e as contas.',
        'A extração gera um Excel com a aba **Resumo** (total por médico) e **uma aba por médico** no formato do processo: ADMISSÃO · PACIENTE · DATA · CONVENIO · DESCRIÇÃO · FAT TOTAL · OPME · imposto · MAT/MED · HM · DEDUÇÕES · INDICAÇÃO · VALOR DE REPASSE.'
      ] },
      { titulo: 'Padrão, LIO e HM (V840)', tipo: 'lista', conteudo: [
        'A **DESCRIÇÃO** é o padrão composto derivado da produção: procedimento + complementos (ISTENT, PRESERFLO…) + lateralidade **OD/OE/AO** extraída do texto (fica de fora quando não dá para saber) + LIO nomeada. Admissão só de exames vira a linha **EXAMES** com o faturamento somado.',
        'Admissão com **LIO** pede marcação **ATLAS × PARCERIA** na Matriz (a ferramenta sugere): PARCERIA deduz o custo da lente e paga o percentual padrão; ATLAS não deduz e paga o percentual "LIO valor ATLAS" (18% na configuração padrão). Em parceria, o custo da lente sai na coluna OPME, como no manual.',
        'O **HM** segue o **cirurgião da produção**: o médico só o recebe quando ele mesmo operou; cirurgião interno/outro → só a indicação. Produção sem cirurgião informado mantém o HM, e o ajuste manual "sem HM" continua valendo.'
      ] }
    ]
  };
  window.AtlasDocs['externos-importar'] = {
    titulo: 'Importar',
    secoes: [
      { titulo: 'Objetivo', tipo: 'paragrafo', conteudo: [
        'Mesma lógica do **Importar QVIS**: o arquivo entra numa **prévia** (quantas admissões novas, quantas já estão na matriz, quantos números foram descartados) e **só grava depois do Confirmar**. Abaixo, o **Status da base** mostra card por arquivo, com a opção de apagar o que veio de cada um.'
      ] },
      { titulo: 'Como operar (passo a passo)', tipo: 'passos', conteudo: [
        'Garanta que a **PRODUÇÃO do mês já foi importada** (modo normal → Importar → Produção).',
        'Solte a planilha recebida — qualquer estrutura serve.',
        'Leia a **prévia**: quantas admissões são novas, quantas já estão na matriz e quantos números foram descartados.',
        'Números descartados são esperados: a varredura pega tudo que parece admissão, e o cruzamento com a PRODUÇÃO separa o que é real.',
        'Confirme. Só depois disso algo é gravado.',
        'Confira o **Status da base**: um card por arquivo, com a opção de apagar o que veio de cada um.'
      ] },
      { titulo: 'Regras', tipo: 'lista', conteudo: [
        'A planilha pode vir em **qualquer estrutura** — a leitura varre todas as células atrás de números de 6 a 10 dígitos.',
        'Só entra o que **existe na PRODUÇÃO** (datas/telefones caem fora sozinhos).',
        'Admissões repetidas não duplicam (a matriz guarda uma vez só).',
        'Se a produção do mês ainda não foi importada, importe-a primeiro (modo normal → Importar → Produção).'
      ] }
    ]
  };
  window.AtlasDocs['externos-medicos'] = {
    titulo: 'Médicos & Clínicas',
    secoes: [
      { titulo: 'Objetivo', tipo: 'paragrafo', conteudo: [
        '**Não recadastra ninguém**: os médicos vêm do cadastro único da ferramenta (módulo MÉDICOS). Aqui você só define o **vínculo** (EXTERNO/HÍBRIDO) e a **clínica** de cada médico — que alimentam o HM e o rótulo da Matriz Externa.'
      ] },
      { titulo: 'Como operar (passo a passo)', tipo: 'passos', conteudo: [
        'Localize o médico (o filtro padrão já mostra externos, híbridos e quem tem admissão na matriz).',
        'Defina o **vínculo**: EXTERNO ou HÍBRIDO. É a decisão de maior impacto — ela determina se o HM soma no fim ou é deduzido antes do percentual.',
        'Vincule a **clínica**, que é o rótulo pelo qual o médico costuma aparecer na planilha recebida.',
        'Clínica nova pode ser criada aqui mesmo ou pela Base de Cálculo.',
        'Cadastro de dados pessoais continua sendo no módulo **MÉDICOS** — aqui só se define vínculo e clínica.'
      ] },
      { titulo: 'Regras', tipo: 'lista', conteudo: [
        'O filtro padrão mostra externos, híbridos e quem tem admissão na matriz — desmarque para ver o cadastro inteiro.',
        'HM só é pago quando o vínculo é **EXTERNO** (o híbrido recebe pela folha interna).',
        'Clínicas novas podem nascer aqui ("＋ Cadastrar nova clínica") ou na Base de Cálculo.'
      ] }
    ]
  };
  window.AtlasDocs['externos-base'] = {
    titulo: 'Base de Cálculo — Externos',
    secoes: [
      { titulo: 'Objetivo', tipo: 'paragrafo', conteudo: [
        'Guardar os valores que alimentam a Matriz Externa: **MAT/MED e HM por procedimento**, **custo das LIOs**, **custo dos OPMEs** e as **clínicas** dos médicos externos. Os **percentuais** (imposto, indicação, indicação LIO ATLAS) moram no botão **Ajustes** do cabeçalho.'
      ] },
      { titulo: 'Como operar (passo a passo)', tipo: 'passos', conteudo: [
        'Atualize **MAT/MED e HM por procedimento** — são deduções e pagamento que entram em toda conta da Matriz Externa.',
        'Atualize o **custo das LIOs**: é ele que a matriz deduz quando a lente é marcada como PARCERIA, e é por ele que a sugestão ATLAS × PARCERIA é feita.',
        'Atualize o **custo dos OPMEs**: a matriz usa esses valores para reconhecer OPME mesmo quando a linha não vem classificada.',
        'Preencha as colunas **AO** quando o valor de ambos os olhos não for simplesmente o dobro; deixando vazio, a matriz usa 2× o unilateral.',
        'Os **percentuais** (imposto, indicação, indicação com LIO ATLAS) ficam no botão **Ajustes** do cabeçalho, não nesta tabela.',
        'Revise tudo **a cada 6 meses**, como manda o processo — base velha erra todas as contas em silêncio.'
      ] },
      { titulo: 'Regras', tipo: 'lista', conteudo: [
        'Regra do processo: **atualizar a cada 6 meses**.',
        'Colunas AO vazias ⇒ a matriz usa 2× o valor unilateral.',
        'Cada cadastro rola dentro do próprio card (o cabeçalho da tabela fica fixo).',
        'O vínculo médico ↔ clínica é feito no cadastro do médico (módulo MÉDICOS).'
      ] }
    ]
  };
  window.AtlasDocs['externos-visao'] = {
    titulo: 'Visão Geral — Externos',
    secoes: [
      { titulo: 'Objetivo', tipo: 'paragrafo', conteudo: [
        '**Em construção.** A estrutura desta visão geral será desenhada junto com o usuário — por enquanto a tela está vazia de propósito.'
      ] }
    ]
  };
})();
