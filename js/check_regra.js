...
(function () {
  ...
  const CSS = `
    ...
    .ckr-modal .btn {
      padding: 8px 16px; border: 1px solid var(--border, #eef0f2); border-radius: 8px;
  ...
  function abrir() {
    ...
    const th = 'padding:6px 8px;background:#5980a6;color:#fff;font-size:11px;text-transform:uppercase;letter-spacing:.04em;white-space:nowrap';
    ...
    const aplicar = () => {
      ...
      const linhas = [];
      if (r.versao != null) {
        badge(vtEl, 'v' + r.versao, '#5980a6', '#FFFFFF');
      ...
      if (!r.temProc) {
        ...
        if (r.semRegra) {
          badge(stEl, 'SEM REGRA', '#a15646', '#FFFFFF');
          vlEl.textContent = 'R$ 0,00';
